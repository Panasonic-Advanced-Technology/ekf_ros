...
入力:
  ...
  /vel   (geometry_msgs/TwistStamped)  GNSSアンテナの速度

観測ベクトル z = [px, py, yaw, vx, vy]
  - px, py  : GNSS を ENU 変換した位置
  - yaw     : 最新の /vel から arctan2(vy, vx) で算出。低速時は状態のyaw
  - vx, vy  : 最新の /vel
"""
...
from geometry_msgs.msg import (
    PoseWithCovarianceStamped, PoseStamped, TwistStamped,
)
...
class EKFROS(Node):
    ...
    def __init__(self):
        ...
        # 最新 /vel をキャッシュ
        self._latest_vel = None  # np.array([vx, vy])
    ...
    def _imu_callback(self, msg):
        ...
        self._ekf.predict(extract_imu_data_2d(msg), dt)
    ...
    def _gnss_callback(self, msg):
        ...
        if self._latest_vel is None:
            self._publish_gnss_enu_pose(gnss_pos, float(self._ekf.x.yaw))
            self._publish_estimated_pose()
            return

        vx, vy = float(self._latest_vel[0]), float(self._latest_vel[1])
        yaw_obs = self._compute_yaw_from_vel(vx, vy, float(self._ekf.x.yaw))

        z = np.array([gnss_pos[0], gnss_pos[1], yaw_obs, vx, vy])
        self._ekf.correct(z)

        self._publish_gnss_enu_pose(gnss_pos, yaw_obs)
        self._publish_estimated_pose()

    def _compute_yaw_from_vel(self, vx, vy, fallback_yaw):
        """速度ベクトルからyawを計算。低速時は fallback_yaw を返す。"""
    ...
    def _initialize_from_gnss(self, msg):
        ...
        self.get_logger().info(
            f'EKF initialized (yaw={init_yaw:.3f}, vel={init_vel.tolist()})')
    ...
    def _publish_estimated_pose(self):
        ...
        msg = make_posestamped_cov_msg_2d(
            pos=x_est.pos, yaw=float(x_est.yaw), cov=P,
            frame_id='map',
            stamp=self.get_clock().now().to_msg(),
        )
        self._estimated_pose_pub.publish(msg)
